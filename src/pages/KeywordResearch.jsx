  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(
        "https://keyword-research3.onrender.com/api/common/analyze-keyword",
        {
          method: "POST",
          headers: {
      ...
      // Store analysis data and scraped keywords separately
      setKeywordData(result.analysisResult);
      setScrapedKeywords(result.scrapedKeywords);
    } catch (error) {
              ...
              <div className="w-full h-full border-1 border-gray-500 p-3 rounded-lg">
                <div className="p-4 shadow-lg rounded-lg">
                  <h1 className="text-lg sm:text-xl font-bold">
                    {scrapedKeywords.length} results for "{searchTerm}"
                  </h1>
                </div>
                <ol className="p-4 max-h-130 overflow-y-auto">
                  {scrapedKeywords.map((item, index) => (
            ...
            <div className="flex flex-col sm:flex-row mt-4">
              ...
              <div className="flex flex-col w-full h-full p-3 rounded-lg">
                <div className="bg-[#12153D] p-4 rounded-lg text-white flex flex-col justify-center items-center">
                  ...
                  <p className="text-3xl mt-2 font-sans font-bold">
                    {keywordData?.keyword_Volume
                      ? keywordData.keyword_Volume.toLocaleString()
                      : "N/A"}
                  </p>
                </div>
                ...
                <div className="p-3 border-1 border-gray-500 rounded-lg mt-4 flex flex-col justify-center items-center">
                  ...
                  <p className="text-3xl mt-2 font-sans font-semibold">
                    {keywordData.keyword_Difficulty}%
                  </p>
                  <p className="text-sm text-center mt-2 font-sans font-semibold">
                    This keyword will demand high-authority referring domains
                    and well-optimized content to start ranking for it.
                  </p>
                </div>
              </div>
                    ...
                    <div className="w-full">
                      ...
                      <div className="relative h-2 rounded-full overflow-hidden mt-4">
                        ...
                        <div
                          ...
                          style={{ left: `${keywordData.spamRiskScore}%` }}
                        />
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex flex-col justify-center items-center bg-[#12153D] text-white p-3 rounded-lg">
                  ...
                  <div className="flex justify-between items-center mt-4 space-x-2">
                    {keywordData?.searchIntent && (
                      <span className="p-1.5 rounded-full text-[#12153D] bg-white text-sm font-bold">
                        {keywordData.searchIntent.charAt(0).toUpperCase() +
                          keywordData.searchIntent.slice(1)}
                      </span>
                    )}
                  </div>
                </div>
