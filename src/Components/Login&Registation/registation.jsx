...
function SignupPage() {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen p-4" style={{ background: 'radial-gradient(80.98% 101.63% at 32.38% 48.5%, rgb(255, 255, 255) 52.19%, rgb(229, 89, 15) 82.56%, rgb(18, 21, 61) 100%)' }}>
      <div className="bg-white p-8 rounded-lg w-full max-w-3xl" style={{ borderRadius: '15px', background: 'radial-gradient(80.98% 101.63% at 32.38% 48.5%, rgb(255, 255, 255) 52.19%, rgb(229, 89, 15) 82.56%, rgb(18, 21, 61) 100%)' }}>
        <div className="absolute inset-0 opacity-50"></div>
        <div className="p-4 rounded-lg w-full max-w-md mx-auto">
          <h2 className="text-2xl font-bold mb-1">Create Account</h2>
          <p className="mb-6 text-md text-gray-500 text-bold">Grow your business like a Raja</p>
          <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 mb-6">
            <button className="flex-1 bg-gray-100 p-2 rounded-lg flex items-center justify-center">
              <svg width="20px" height="20px" viewBox="0 0 32 32" data-name="Layer 1" id="Layer_1" xmlns="http://www.w3.org/2000/svg">
            ...
            <button className="flex-1 bg-gray-100 p-2 rounded-lg flex items-center justify-center">
              <svg width="20px" height="20px" viewBox="-1.5 0 20 20" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
          ...
          <div className="mb-4">
            ...
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full p-2 border border-gray-300 rounded-lg" />
          </div>
          <div className="mb-4">
            <label className="block text-gray-700">Create Password</label>
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full p-2 border border-gray-300 rounded-lg" />
          </div>
          <div className="mb-6 flex items-center">
            <input type="checkbox" checked={agree} onChange={(e) => setAgree(e.target.checked)} className="mr-2" />
            <label className="text-gray-700">I agree with privacy and policy</label>
          </div>
          <button onClick={handleSignup} className="w-full bg-blue-900 text-white p-2 rounded-lg">Sign Up</button>
        </div>
      </div>
    </div>
  );
